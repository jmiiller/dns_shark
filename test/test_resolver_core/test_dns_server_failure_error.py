...
class DNSServerFailureTest(unittest.TestCase):
    """
    ...
    def setUpClass(cls):
        ...
        """
        cls.response_with_server_failure_error: bytes = bytes.fromhex('19b6840200010001000300030377777702637303756263026361000001'
        ...
        cls.mock_socket: Mock = Mock(**{'recv.return_value': cls.response_with_server_failure_error})

        cls.mock_random: Mock = Mock(**{'randint.return_value': 6582})

    def test_server_failure_error(self):
        """
        Test case for when the first response given by the queried dns name server gives a server failure error.
        """

        resolver: ResolverCore = ResolverCore(self.mock_socket, False, "1.2.3.4", self.mock_random)

        self.assertRaises(DNSServerFailureError, resolver.resolve_domain_name, "www.cs.ubc.ca", "1.2.3.4", 1)
