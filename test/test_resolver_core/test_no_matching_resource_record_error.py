...
class DNSNoMatchingResourceRecordTest(unittest.TestCase):
    """
    ...
    def setUpClass(cls):
        ...
        """
        cls.response_with_no_matching_resource_record_error: bytes = bytes.fromhex('19b6840000010001000300030377777702637303756263026361000001'
        ...
        cls.mock_socket: Mock = Mock(**{'recv.return_value': cls.response_with_no_matching_resource_record_error})

        cls.mock_random: Mock = Mock(**{'randint.return_value': 6582})

    def test_no_matching_resource_record_error(self):
        """
        Test case for when the first response given by the queried dns name server finds records for the searched for
        domain name. However, none of these records match the desired record types.
        """

        resolver: ResolverCore = ResolverCore(self.mock_socket, False, "1.2.3.4", self.mock_random)

        self.assertRaises(DNSNoMatchingResourceRecordError, resolver.resolve_domain_name, "www.cs.ubc.ca", "1.2.3.4", 28)
